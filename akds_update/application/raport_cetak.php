...
?>
            <div class="col-xs-12">  
              <div class="box">
                ...
                $pengetahuan = mysql_fetch_array(mysql_query("SELECT sum(z.raport) as total FROM (SELECT sum((a.nilai1+a.nilai2+a.nilai3+a.nilai4+a.nilai5)/5)/count(a.nisn) as raport FROM rb_nilai_pengetahuan a JOIN rb_jadwal_pelajaran b ON a.kodejdwl=b.kodejdwl where a.nisn='$r[nisn]' AND b.id_tahun_akademik='$_GET[tahun]' GROUP BY a.kodejdwl) as z"));
                 
                $keterampilan = mysql_fetch_array(mysql_query("SELECT sum(z.raport) as total FROM (SELECT sum(GREATEST(a.nilai1,a.nilai2,a.nilai3,a.nilai4,a.nilai5,a.nilai6))/count(a.nisn) as raport FROM rb_nilai_keterampilan a JOIN rb_jadwal_pelajaran b ON a.kodejdwl=b.kodejdwl where a.nisn='$r[nisn]' AND b.id_tahun_akademik='$_GET[tahun]' GROUP BY a.kodejdwl) as z"));

                $nilai_total = number_format($pengetahuan[total])+number_format($keterampilan[total]);// menjumlahkan nilai raport
                ...
                mysql_query("INSERT INTO rb_peringkat VALUES('','$r[nisn]','$r[kode_kelas]','$_GET[tahun]','$nilai_total','')");
                    ...
                    echo "<tr><td width=40px>$no</td>
                              ...
                              <td>$r[nisn]</td>
                              <td>$r[nama]</td>
                              <td align=center>$r[kode_kelas]</td>
                ...
                $rank = mysql_query("SELECT * FROM rb_peringkat where kode_kelas='$_GET[id]' AND id_tahun_akademik='$_GET[tahun]' ORDER BY nilai_total DESC");// query untuk merangking siswa
                      $ii = 1;
