...
$row1 = mysql_fetch_array(mysql_query("SELECT count(*) as total_sakit FROM `rb_absensi_siswa` a JOIN rb_jadwal_pelajaran b ON a.kodejdwl=b.kodejdwl 
where a.nisn='$_GET[id]' AND b.id_tahun_akademik='$_GET[tahun]' AND a.kode_kehadiran='S'"));
 
$row2 = mysql_fetch_array(mysql_query("SELECT count(*) as total_izin FROM `rb_absensi_siswa` a JOIN rb_jadwal_pelajaran b ON a.kodejdwl=b.kodejdwl 
where a.nisn='$_GET[id]' AND b.id_tahun_akademik='$_GET[tahun]' AND a.kode_kehadiran='I'"));
 
$row3 = mysql_fetch_array(mysql_query("SELECT count(*) as total_alpa FROM `rb_absensi_siswa` a JOIN rb_jadwal_pelajaran b ON a.kodejdwl=b.kodejdwl 
where a.nisn='$_GET[id]' AND b.id_tahun_akademik='$_GET[tahun]' AND a.kode_kehadiran='A'"));

$ctt = mysql_fetch_array(mysql_query("SELECT * FROM rb_nilai_catatan_wakel where nisn='$_GET[id]' AND id_tahun_akademik='$_GET[tahun]'"));
 
echo "<b>E. Ketidakhadiran</b>
      <table id='tablemodul1' width=85% border=1>
        <tr><td width='70%'>Sakit</td>  <td width='10px'> : </td> <td align=center>$row1[total_sakit]</td> </tr>
        <tr><td>Izin</td>               <td> : </td>              <td align=center>$row2[total_izin]</td> </tr>
        <tr><td>Tanpa Keterangan</td>   <td> : </td>              <td align=center>$row3[total_alpa]</td> </tr>
      </table>";
...
<table border=0 width=100%>
  <tr>
    ...
    <td width="520"align="center">Mengetahui <br> Kepala SMK Plus AL-MAFTUH,</td>
    <td width="260" align="left">Sukabumi, <?php echo tgl_raport(date("Y-m-d")); ?> <br> Wali Kelas</td>
  </tr>
  <tr>
